...
####  1) 1-D L94 vertical advection: advect1d_l94() (Lin et al., 1994)
####  2) 1-D PPM vertical advection: advect1d_ppm() (Collela and Woodward, 1984)
####  The vertical advection conserve pi*q rather than q. (Lin and Rood, 1996)
...
function advect1d_vertical_L94(s1d_in,vel_1d,nz,Δt,p)
    ...
    U[1] = 0; U[nz+1] = 0;  ## Dirichlet BC with zero on top and bottom of the column.
    ...
    ΔP[1] = ΔP[2]; ΔP[nz+2] = ΔP[nz+1]
    
    
    ϕ1[3:nz+2] = s1d_in[:] #.* p[1:nz]
    ϕ1[1] = ϕ1[3]; ϕ1[2] = ϕ1[3];
    ϕ1[nz+3] = ϕ1[nz+2]; ϕ1[nz+4] = ϕ1[nz+2]; ϕ1[nz+5] = ϕ1[nz+2];
...
function advect1d_vertical_ppm(s1d_in::Vector{Float64},vel_1d::Vector{Float64},nz,Δt,p)
    ...
    ΔP[1] = ΔP[4]; ΔP[2] = ΔP[4]; ΔP[3] = ΔP[4]; 
    ΔP[nz+4] = ΔP[nz+3]; ΔP[nz+5] = ΔP[nz+3]; ΔP[nz+6] = ΔP[nz+3]; ΔP[nz+7] = ΔP[nz+3]
    ...
    for i in 2:nz+5
        if (ϕ1[i+1]-ϕ1[i])*(ϕ1[i]-ϕ1[i-1]) > 0
            δₘϕ[i] = min(abs(δϕ[i]), 2*abs(ϕ1[i]-ϕ1[i-1]), 2*abs(ϕ1[i+1]-ϕ1[i])) * sign(δϕ[i])
    ...
    for i in 2:nz+5
        ...
        ϕ₊½[i] = ϕ1[i] + 
                ...
                ((ΔP[i-1]+ΔP[i])/(2*ΔP[i]+ΔP[i+1])-(ΔP[i+2]+ΔP[i+1])/(2*ΔP[i+1]+ΔP[i]))*
                (ϕ1[i+1]-ϕ1[i]) - 
    ...
    for i in 2:nz+5
        ...
        δ²ϕ[i] = 1/(ΔP[i-1]+ΔP[i]+ΔP[i+1])*((ϕ1[i+1]-ϕ1[i])/(ΔP[i+1]+ΔP[i])-(ϕ1[i]-ϕ1[i-1])/(ΔP[i]+ΔP[i-1]))
    end
    
    for i in 2:nz+5
        if -δ²ϕ[i+1]*δ²ϕ[i-1]*abs(ϕ1[i+1]-ϕ1[i-1]) - ϵ*min(abs(ϕ1[i+1]),abs(ϕ1[i-1])) > 0
            #η_tilde[i] = -(δ²ϕ[i+1]-δ²ϕ[i-1]) * (ΔP[i-1]^2) / (ϕ1[i+1]-ϕ1[i-1])
            η̃[i] = -(δ²ϕ[i+1]-δ²ϕ[i-1])/(ΔP[i+1]+ΔP[i])*(ΔP[i]^3+ΔP[i-1]^3)/(ϕ1[i+1]-ϕ1[i-1])
        else
            η̃[i] = 0
        end
    end
    ...
    for i in 3:nz+3
        if U[i-2] >= 0
            ...
            FLUX[i-2] = courant * (ϕR[i]-1/2*courant*(Δϕ[i]-(1-2/3*courant)*ϕ₆[i]))
        else
            courant = U[i-2]*Δt/ΔP[i]
            FLUX[i-2] = courant * (ϕL[i+1]-1/2*courant*(Δϕ[i+1]-(1+2/3*courant)*ϕ₆[i+1]))
        end
    end
